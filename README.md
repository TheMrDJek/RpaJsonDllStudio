...
1. Скачайте последний релиз из раздела [Releases](https://github.com/themrdjek/RpaJsonDllStudio/releases)
2. Распакуйте архив в любую папку
3. Запустите файл `RpaJsonDllStudio.exe`

### macOS

1. Скачайте последний релиз для macOS из раздела [Releases](https://github.com/themrdjek/RpaJsonDllStudio/releases)
2. Распакуйте архив
...
1. Скачайте последний релиз для Linux из раздела [Releases](https://github.com/themrdjek/RpaJsonDllStudio/releases)
2. Распакуйте архив
3. Дайте файлу права на выполнение: `chmod +x RpaJsonDllStudio`
4. Запустите приложение: `./RpaJsonDllStudio`
...
![Интерфейс RpaJsonDllStudio](common/interfaceMain1.png)
...
![Корректный C# код](common/intrfaceValid1.png)
...
![Ошибки в C# коде](common/interfaceNotValid1.png)
...
Приложение можно опубликовать в виде одного исполняемого файла (exe), который не требует установки:

```bash
# Публикация для Windows
dotnet publish src/RpaJsonDllStudio/RpaJsonDllStudio.csproj -c Release -r win-x64 --self-contained -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:PublishTrimmed=true -o ./publish

# Публикация для macOS
dotnet publish src/RpaJsonDllStudio/RpaJsonDllStudio.csproj -c Release -r osx-x64 --self-contained -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:PublishTrimmed=true -o ./publish

# Публикация для Linux
dotnet publish src/RpaJsonDllStudio/RpaJsonDllStudio.csproj -c Release -r linux-x64 --self-contained -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:PublishTrimmed=true -o ./publish
```

Параметры:
- `-c Release` - сборка в режиме Release
- `-r [runtime]` - целевая платформа (win-x64, osx-x64, linux-x64)
- `--self-contained` - включает все зависимости в сборку
- `-p:PublishSingleFile=true` - упаковывает всё в один файл
- `-p:PublishReadyToRun=true` - предварительная компиляция для улучшения производительности
- `-p:PublishTrimmed=true` - удаление неиспользуемого кода для уменьшения размера

Также можно добавить эти настройки в файл проекта:
